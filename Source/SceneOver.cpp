	...
	loading_thread = std::make_unique<std::thread>([&](ID3D11Device* device)
		{
			...
			overTex = std::make_unique<Sprite>(device, L"Data/images/GAME OVER.png");
			...
			dissolveColor = DirectX::XMFLOAT4(1, 0.3f, 0, 1);
			dissolveThreshold = 0;
...
int SceneOver::Update(float elapsed_time)
{
	if (IsNowLoading())
	{
		return 0;
	...
	if (dissolveFlag)
	{
		...
		overTex->SetDissolveColor(dissolveColor);
		overTex->SetDissolveThreshold(dissolveThreshold);
		overTex->SetEmissiveWidth(dissolveEmissiveWidth);
...
void SceneOver::Render(float elapsed_time, ID3D11DeviceContext* devicecontext)
{
	if (IsNowLoading())
	{
	...
	frameBuffer[0]->Activate(devicecontext);
	{
		...
		overTex->DissolveRender(devicecontext, dissolveShader.get(), srv.Get(), 308, 300, 1304, 136, 0, 0, 1304, 136, 0, 1, 1, 1, 1);
