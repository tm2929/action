...
{
	max -= min;
	return -max * cos(t * DirectX::XMConvertToRadians(90) / totaltime) + max + min;
}

float OutSine(float t, float totaltime, float max, float min)
{
	max -= min;
	return max * sin(t * DirectX::XMConvertToRadians(90) / totaltime) + min;
}
...
float InOutExp(float t, float totaltime, float max, float min)
{
	...
	max -= min;
	t /= totaltime;
	if (t / 2 < 1) return max / 2 * powf(2, 10 * (t - 1)) + min;
	--t;
...
{
	if (t < totaltime / 2) return InBounce(t * 2, totaltime, max - min, max) * 0.5f + min;
	else return OutBounce(t * 2 - totaltime, totaltime, max - min, 0) * 0.5f + min + (max - min) * 0.5f;
}
